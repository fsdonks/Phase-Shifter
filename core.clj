...
(defn demands-new-phases [demandfile changefile]
  (let [demandheader (read-header demandfile) changeheader (read-header changefile)
        ...
        _ (def p (change-phase phases phaseheader changes changeheader))
...
;; Displays file select GUIs and formats and saves new demandfile.
(defn ->shift-demand []
  (let [demandfile (first (choose-file "SELECT Demand File (.txt)"))
        changefile (first (choose-file "SELECT Change File (.txt)"))
        outfile (str (first (clojure.string/split demandfile #".txt")) "_SHIFTED.txt")]
    (when (and demandfile changefile)
      (->output-new-phases demandfile (read-header demandfile) changefile outfile)
      (println (str "New Demand File created at: " outfile))
      (->frame "File Created" (str "New Demand File created at\n" outfile)))))

(defn -main [& args]
  (let [f (try (->shift-demand)
            (catch Exception e (->frame "Clojure Error" (str e "\nCause: " (:cause e) "\n\nTrace:\n" (:trace e)))))]))
